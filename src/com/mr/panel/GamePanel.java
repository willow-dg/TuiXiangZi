...
public class GamePanel extends JPanel implements KeyListener, MouseListener, MouseMotionListener {
    ...
    private ArrayList<com.mr.model.Box> boxes;//箱子列表
    ...
    private void moveThePlayer(int xNext1, int xNext2, int yNext1, int yNext2) {
        ...
        if (boxes.contains(box)) {//ruguo这个箱子在箱子列表中是存在的
            ...
            if (boxes.contains(new com.mr.model.Box(xNext2, yNext2))) {//如果箱子钱还是箱子
                return;//也是不用做
            }
            if (data[xNext2][yNext2] instanceof Destination) {//如果箱子前方是目的地
                box.arrive();//箱子到达
    ...
    private void paintImage() {
        ...
        for (com.mr.model.Box box : boxes) {//便利所有箱子
            //在对应位置上绘制图片
    ...
    public void keyPressed(KeyEvent e) {
        ...
        switch (key) {
            ...
            case KeyEvent.VK_RIGHT:
                moveThePlayer(x + 1, y, x + 2, y);
                break;
            case KeyEvent.VK_ESCAPE://如果用户按下的是ESC
